...
struct CounterHandler {
    ...
    state: Arc<CounterStateController>,
}
...
impl Handler for CounterHandler {
    fn new(state_update_channel: StateUpdateChannel) -> Self {
        Self {
            state: Arc::new(CounterStateController::new(state_update_channel)),
        }
    }
...
/// The StateController is a server-side wrapper around the user's state. It manages the 
/// state under a [parking_lot::Mutex] and stores a [mpsc::Sender] to send any changes.
/// This is accessable through self.state for RPC functions.
struct CounterStateController {
...
impl CounterStateController {
    ...
    /// Locks the state to the current RPC handler by providing a StateGuard.
...
/// An RPC client wrapping the default runtime client using rustls and tungstenite, 
/// implementing the [Counter] trait.
